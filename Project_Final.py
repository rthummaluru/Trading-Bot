...
def thresholds(data, target_date):
    ...
    # Reverse the data to have it in chronological order for calculation
    data = data[::-1]
    
    # Convert date strings to datetime for comparison and indexing
    data_dates = [datetime.strptime(x[1], '%Y-%m-%d') for x in data]
    target_date_dt = datetime.strptime(target_date, '%Y-%m-%d')
    
    # Find index of the target_date in data
    try:
        target_index = data_dates.index(target_date_dt)
    except ValueError:
        # Return None if the target_date is not in data
        return {'5-day MA': None, '20-day MA': None}
    
    # Calculate moving averages for 5-day and 20-day periods directly from prices
    moving_averages = {'5-day MA': None, '20-day MA': None}
    for key in moving_averages.keys():
        days = int(key.split('-')[0])
        if target_index + days <= len(data):
            relevant_prices = [data[i][2] for i in range(target_index, target_index + days)]
            ma = sum(relevant_prices) / days
            moving_averages[key] = ma
    
    return moving_averages
test_data = price_info('PBBOCARQLUUHKQBA', 50, "META")
test_thresh = thresholds(test_data, '2024-02-12')
test_thresh
...
def signals(data, target_date, thresholds):
    ...
    trade_action = "Do nothing"
    
    # Find the data for the target date
    daily_data = next((item for item in data if item[1] == target_date), None)
    if daily_data is None:
        return {"trade": "Data not available for the specified date"}
    
    # Extract the adjusted closing price for the target date
    adj_close_price = daily_data[2]
    
    # Initialize signals as None
    I1, I2 = None, None
    
   # Only calculate signals if thresholds are not None
    if thresholds['5-day MA'] is not None:
        I1 = 1 if adj_close_price > thresholds['5-day MA'] else -1
    if thresholds['20-day MA'] is not None:
        I2 = 1 if adj_close_price > thresholds['20-day MA'] else -1

     # Example handling
    if I1 is None or I2 is None:
        I3 = None  # Or however you want to handle partial or missing signals
    else:
        I3 = I1 + I2  # Or your logic for combining signals
    
    # Determine the trading signal based on I3
    if I3 is None or I3 <= -2:
        trade_action = "Sell"
    elif I3 >= 2:
        trade_action = "Buy"
    
    # Return the trade action in a dictionary
